...
std::string_view FznsoModel::constraint_ident(fznso::Constraint constraint) const {
  // A declared constraint is registered with the flattener as `fzn_<ident>` as
  // well as `<ident>`, and the library call reaches the solver under the
  // prefixed name. The solver declared, and must be handed, the bare one.
  // No registry name starts with `fzn_`, and every other name carries the
  // solver's own prefix, so stripping cannot turn one name into another.
  std::string_view ident = view(_constraints[constraint.index]->id());
  constexpr std::string_view prefix = "fzn_";
  if (ident.substr(0, prefix.size()) == prefix) {
    ident.remove_prefix(prefix.size());
  }
  return ident;
}
