  ...
  interpretCombinator(Expression* comb, Env& env, SolverInstanceBase* solver) {
    ...
    if(Call* call = comb->dyn_cast<Call>()) {      
      if(call->id().str() == constants().combinators.and_.str()) {
        ...
        // TODO: AND combinator takes an array as argument
        for(unsigned int i=0; i<call->args().size(); i++) {
          SolverInstance::Status status = interpretCombinator(call->args()[i],env,solver);
        ...
        // TODO: OR combinator takes an array as argument
        for(unsigned int i=0; i<call->args().size(); i++) {
          status = interpretCombinator(call->args()[i],env,solver);
          if(status == SolverInstance::SAT)
            oneIsFeasible = true;
        }
        // TODO: not sure about the behaviour of OR combinator...
