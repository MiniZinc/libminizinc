...
void finalise_output(EnvI& e) {
  if (!e.output->empty()) {
    ...
    std::vector<VarDecl*> rhs_replace;
    for (unsigned int i = 0; i < e.output->size(); i++) {
              ...
              if (it != e.reverseMappers.end()) {
                ...
                if (Id* ident = Expression::dynamicCast<Id>(reallyFlat->e())) {
                  if (e.reverseMappers.find(ident) != e.reverseMappers.end()) {
                    ...
                    Id* cp = Expression::cast<Id>(copy(e, e.cmap, ident));
                    vd->e(cp);
                    ...
                    if (vd->e() == cp) {
                      // Right hand side still points to a value in the FlatZinc eventhough it is
                      // reversed mapped.
                      rhs_replace.push_back(vd);
                    }
    ...
    for (auto& vd : rhs_replace) {
      auto* ident = Expression::cast<Id>(vd->e());
      auto find = e.outputVarOccurrences.find(ident->decl());
      if (find >= 0) {
        auto* out = (*e.output)[find]->cast<VarDeclI>()->e();
        vd->e(out->id());
      }
    }
  }
