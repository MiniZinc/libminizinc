...
EE flatten_call(EnvI& env, Ctx ctx, Expression* e, VarDecl* r, VarDecl* b) {
  ...
  if (ctx.b == C_ROOT && decl->e() == nullptr && cid == constants().ids.forall &&
    ...
    if (cid == constants().ids.lin_exp && c->type().isint()) {
      ...
      if (cid == constants().ids.clause && c->arg(0)->isa<ArrayLit>() &&
        ...
        if (al->flat()) {
          ...
          CallArgItem cai(env);
          for (unsigned int i = 0; i < al->size(); i++) {
            EE res = flat_exp(env, argctx, (*al)[i], nullptr, constants().var_true);
            if (res.r()->type().ispar()) {
    ...
    if (isPartial && c->type().isbool() && !c->type().isopt()) {
      ret.b = bind(env, Ctx(), b, constants().lit_true);
      args_ee.resize(1);
      args_ee[0] = EE(nullptr, constants().lit_false);
      ...
      while (!pos_stack.empty() || !neg_stack.empty()) {
        while (!pos_stack.empty()) {
          ...
          if (cur->isa<Id>() && seen.find(cur) != seen.end()) {
            ...
            GCLock lock;
            if (Call* sc =
                    Expression::dyn_cast<Call>(same_call(env, cur, constants().ids.exists))) {
              GCLock lock;
              ArrayLit* sc_c = eval_array_lit(env, sc->arg(0));
              for (unsigned int j = 0; j < sc_c->size(); j++) {
                pos_stack.push_back((*sc_c)[j]);
              }
            } else if (Call* sc = Expression::dyn_cast<Call>(
              ...
              Call* not_call =
                  Expression::dyn_cast<Call>(same_call(env, cur, constants().ids.bool_not));
              if ((eq_call != nullptr) &&
                  Expression::equal(eq_call->arg(1), constants().lit_false)) {
                neg_stack.push_back(eq_call->arg(0));
              } else if ((eq_call != nullptr) &&
                         Expression::equal(eq_call->arg(0), constants().lit_false)) {
                neg_stack.push_back(eq_call->arg(1));
              } else if ((eq_call != nullptr) &&
                         Expression::equal(eq_call->arg(1), constants().lit_true)) {
                pos_stack.push_back(eq_call->arg(0));
              } else if ((eq_call != nullptr) &&
                         Expression::equal(eq_call->arg(0), constants().lit_true)) {
                pos_stack.push_back(eq_call->arg(1));
              } else if ((not_call != nullptr) && not_call->n_args() == 1) {
        ...
        while (!neg_stack.empty()) {
          ...
          if (cur->isa<Id>() && seen.find(cur) != seen.end()) {
            ...
            seen.insert(cur);
            if (Call* sc =
                    Expression::dyn_cast<Call>(same_call(env, cur, constants().ids.forall))) {
              GCLock lock;
              ArrayLit* sc_c = eval_array_lit(env, sc->arg(0));
              for (unsigned int j = 0; j < sc_c->size(); j++) {
                neg_stack.push_back((*sc_c)[j]);
            ...
            if (args[i]()->type().bt() == Type::BT_INT) {
              ...
              if (needToConstrain) {
                ...
                if (ctx.b == C_ROOT) {
                  ...
                  EE ee = flat_exp(env, Ctx(), domconstraint, nullptr, constants().var_true);
                  ee.b = ee.r;
    ...
    call_nonreif:
      if (decl->e() == nullptr ||
        ...
        } else if (decl->_builtins.e != nullptr) {
          KeepAlive callres;
          {
            GCLock lock;
        ...
        if (decl->e()->type().isbool() && !decl->e()->type().isopt()) {
          ...
          if (ctx.b == C_ROOT && r == constants().var_true) {
            ...
            EE ee = flat_exp(env, nctx, decl->e(), nullptr, constants().var_true);
            ee.b = ee.r;
            args_ee.push_back(ee);
          }
          ret.r = conj(env, r, ctx, args_ee);
