...
void enable_native_predicates(Env& env, const NativePredicates& np) {
  ...
  /// Whether \a fi is the rewrite layer's rename of \a ident onto the
  /// library-facing `fzn_<ident>`, rather than a decomposition of it.
  ///
  /// Such a rename keeps its body. It is not a decomposition to be replaced —
  /// it is the step that gives the constraint the name every FZnSO library uses
  /// for it, and `fzn_<ident>` is the declaration the list below makes native.
  /// Dropping it instead would hand the solver the bare FlatZinc builtin while
  /// everything reaching it through a library arrived as `fzn_<ident>`, so the
  /// same constraint would appear under two spellings depending on whether the
  /// model named it directly.
  auto is_rename_onto_fzn = [](FunctionI* fi, const std::string& ident) {
    if (ident.compare(0, 4, "fzn_") == 0) {
      return false;
    }
    Call* body = Expression::dynamicCast<Call>(fi->e());
    return body != nullptr && std::string(body->id().c_str()) == "fzn_" + ident;
  };
...
void declare_solver_constraints(Env& env, const NativePredicates& np,
  ...
  public:
    ...
    void vFunctionI(FunctionI* fi) {
      ...
      // items.
      // A rewrite that could carry a half-reified form: the shape checks in
      // `declare_solver_constraints` decide. Not filtered on a `_reif` suffix,
      // because `array_bool_or` and `bool_or` are reified forms without one.
      if (fi->e() != nullptr && fi->paramCount() > 0 &&
          fi->param(fi->paramCount() - 1)->type().isvarbool() &&
          fi->param(fi->paramCount() - 1)->type().dim() == 0) {
  ...
  for (FunctionI* fi : scan.reified) {
    ...
    const bool targetReif =
        target.size() > 5 && target.compare(target.size() - 5, 5, "_reif") == 0;
    Id* last = Expression::dynamicCast<Id>(body->arg(body->argCount() - 1));
    if (last == nullptr || last->str() != fi->param(fi->paramCount() - 1)->id()->str()) {
      continue;
    }
    // `array_bool_and` lowers onto `fzn_bool_array_and`, which is a reified
    // form under a name that does not say so, so the suffix is stripped only
    // where there is one.
    std::string targetImp =
        targetReif ? target.substr(0, target.size() - 5) + "_imp" : target + "_imp";
    // Either the solver posts the half-reified form itself, or a library file
    // has a body for it. The second is the ordinary case: a solver that pays
    // for reification states `b -> c` in its own `fzn_<ident>_imp`, and this is
    // what carries the rewrite through to it.
    if (solverDeclares.count(targetImp) == 0 && scan.declared.count(targetImp) == 0) {
      continue;
    }
    std::string nameImp = name.size() > 5 && name.compare(name.size() - 5, 5, "_reif") == 0
                              ? name.substr(0, name.size() - 5) + "_imp"
                              : name + "_imp";
